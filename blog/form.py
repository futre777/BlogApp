...
class MessageForm(forms.ModelForm):
	def __init__(self, post_id, *args, **kwargs):
		...
		self.fields['mensagem'].widget.attrs.update({'id':'message','cols':'30', 'rows':'4', 'class':'form-control', 'placeholder':'Escreve teus notes ou perguntas aqui...'})
		self.fields['post'] = forms.CharField(initial=p.titulo)
		self.fields['post'].widget = forms.HiddenInput()

	class Meta:
		model = BlogMessage
		fields = ['nome', 'email', 'mensagem', 'post'] 
